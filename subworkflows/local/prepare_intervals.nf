...
//
// PREPARE INTERVALS
// COPIED FROM THE SAREK PIPELINE

// Initialize channels based on params or indices that were just built
// For all modules here:
// A when clause condition is defined in the conf/modules.config to determine if the module should be run

include { BUILD_INTERVALS                                     } from "${projectDir}/modules/local/build_intervals/main"
include { CREATE_INTERVALS_BED                                } from "${projectDir}/modules/local/create_intervals_bed/main"
include { GATK4_INTERVALLISTTOBED                             } from "${projectDir}/modules/nf-core/modules/gatk4/intervallisttobed/main"
include { TABIX_BGZIPTABIX as TABIX_BGZIPTABIX_INTERVAL_SPLIT } from "${projectDir}/modules/nf-core/modules/tabix/bgziptabix/main"
...
workflow PREPARE_INTERVALS {
    ...
    if (params.no_intervals) {
        ...
        ch_intervals_bed_gz_tbi =
            TABIX_BGZIPTABIX_INTERVAL_SPLIT.out.gz_tbi
            .map{ meta, bed, tbi -> [bed, tbi ]}.toList().map{
                                        it ->
                                        [it, it.size()] // Adding number of intervals as elements
                                    }.transpose()
